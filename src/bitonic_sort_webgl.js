...
import {PassThru} from './pass_thru'
...
void main()	{
  ...
  // y 成分をソートのキーとして使用
  vec4 cmin = (val0.y < val1.y) ? val0: val1; // 小さいほう
  vec4 cmax = (val0.y < val1.y) ? val1: val0; // 大きいほう
...
export class BitonicSort {
  ...
  writeDataTexture(src) {
    ...
    for (let i = 0, length = buffer.length / 4; i < length; i++) {
      let offset = i * 4;
      let value = src[i] || 0;
      buffer[offset] = value;
